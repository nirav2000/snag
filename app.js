const APP_BUILD='2026.09.22.1145';
...
async function initFirebase(cfg){
  ...
  try{
    ...
    ]),10000,'Firebase library load');
    const [appMod,fsMod,authMod]=mods;
    cloudStatus={state:'starting',message:'Starting Firebase authentication…'};render();
    ...
    try{await withTimeout(auth.authStateReady(),5000,'Firebase auth state');}catch(e){console.warn(e)}
    if(!auth.currentUser){
      cloudStatus={state:'starting',message:'Signing in securely…'};render();
      await withTimeout(authMod.signInAnonymously(auth),8000,'Anonymous sign-in');
    }
    ...
    if(launchProjectId&&launchInviteId){
      await withTimeout(joinInvitedProject(launchProjectId,launchInviteId),10000,'Project invite');
      cloudStatus={state:'connected',message:`Shared cloud connected · invite access · R2 ${window.SNAG_R2_API?'configured':'not configured'}`};
    }else{
      await withTimeout(ensureProjectRemote(),10000,'Project setup');
      let migration={written:0,total:0,mediaFailed:0};
      try{migration=await withTimeout(migrateLocalProjectToCloud(),12000,'Local snag sync')}catch(e){console.warn(e)}
    ...
    subscribeFirebase().catch(e=>console.warn('Subscription start failed',e));
    if(!launchInviteId)setTimeout(()=>maybeShowFirstGuide('owner'),700);
    render();
  }catch(e){
    console.error('Firebase startup failed',e);
    cloudStatus={state:'error',message:`${e?.code||e?.message||'Firebase connection failed'} · app remains available locally`};
    render();
    throw e;
  }
}
