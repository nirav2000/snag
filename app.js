const APP_BUILD='2026.09.21.1635';
...
function render(){renderProject();renderStats();renderFilters();renderRecentActivity();renderList();renderProjects();renderSettings();renderQuickSuggestions();renderCloudDiagnostics();}
...
function renderStats(){const s=visibleProjectSnags();$('statActive').textContent=s.filter(x=>x.status!=='resolved'&&!x.archived).length;$('statProgress').textContent=s.filter(x=>x.status==='in-progress'&&!x.archived).length;$('statReview').textContent=s.filter(x=>x.status==='review'&&!x.archived).length;$('statResolved').textContent=s.filter(x=>x.status==='resolved').length;document.querySelectorAll('.stat-card').forEach(x=>x.classList.toggle('active',x.dataset.statFilter===view.status));}
...
function renderRecentActivity(){
  ...
  items.sort((x,y)=>new Date(y.at)-new Date(x.at));const recent=items.slice(0,5);
  host.innerHTML=recent.length?recent.map(x=>`<button class="activity-row" type="button" data-activity-snag="${x.s.id}"><span class="new-dot ${x.u?'is-new':''}"></span>${activityThumb(x.s,x.u)}<span class="activity-copy"><strong>${escapeHtml(x.s.ref)} · ${escapeHtml(x.s.title)}</strong><small>${escapeHtml(x.label||'Update')} · ${fmt(x.at)}</small></span></button>`).join(''):'<div class="empty-inline">Nothing new.</div>';
  host.querySelectorAll('[data-activity-snag]').forEach(b=>b.onclick=()=>openDetail(b.dataset.activitySnag));
  const summary=$('newActivitySummary');if(summary)summary.textContent=recent.length?`${recent.length} recent item${recent.length===1?'':'s'} · newest ${fmt(recent[0].at)}`:'No new activity';
}
...
async function initFirebase(cfg){
  ...
  else {await ensureProjectRemote();const migration=await migrateLocalProjectToCloud();cloudStatus={state:'connected',message:`Shared cloud connected · owner access · ${migration.written}/${migration.total} local snags synced${migration.mediaFailed?` · ${migration.mediaFailed} media item group(s) pending`:''} · R2 ${window.SNAG_R2_API?'configured':'not configured'}`};}
  subscribeFirebase();
  if(launchInviteId)cloudStatus={state:'connected',message:`Shared cloud connected · invite access · R2 ${window.SNAG_R2_API?'configured':'not configured'}`};render();
}
...
async function subscribeFirebase(){if(!firebase?.auth?.currentUser)return;unsubscribe?.();await loadCurrentMember();const {fsMod,db}=firebase;const q=fsMod.query(fsMod.collection(db,'snag_projects',selectedProjectId,'snags'),fsMod.orderBy('updatedAt','desc'));unsubscribe=fsMod.onSnapshot(q,async snap=>{for(const d of snap.docs){const data={id:d.id,...d.data()};const us=await fsMod.getDocs(fsMod.collection(db,'snag_projects',selectedProjectId,'snags',d.id,'updates'));data.updates=us.docs.map(x=>({id:x.id,...x.data()}));const i=state.snags.findIndex(x=>x.id===data.id);if(i>=0)state.snags[i]=data;else state.snags.push(data);}saveState();render();if(detailId)openDetail(detailId);},e=>console.warn('Firestore listener',e));}
