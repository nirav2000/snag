...
async function prepareMedia(files,pathPrefix){const result=[];for(const file of files){if(firebase){const {storageMod,storage}=firebase;const r=storageMod.ref(storage,`${pathPrefix}/${uid()}-${file.name.replace(/[^a-zA-Z0-9._-]/g,'_')}`);await storageMod.uploadBytes(r,file);result.push({url:await storageMod.getDownloadURL(r),name:file.name,type:file.type,size:file.size,local:false});}else result.push(await fileToDataUrl(file));}return result;}
...
async function initFirebase(cfg){const [appMod,fsMod,storageMod,authMod]=await Promise.all([import(`https://www.gstatic.com/firebasejs/${FIREBASE_VERSION}/firebase-app.js`),import(`https://www.gstatic.com/firebasejs/${FIREBASE_VERSION}/firebase-firestore.js`),import(`https://www.gstatic.com/firebasejs/${FIREBASE_VERSION}/firebase-storage.js`),import(`https://www.gstatic.com/firebasejs/${FIREBASE_VERSION}/firebase-auth.js`)]);const app=appMod.initializeApp(cfg);const db=fsMod.getFirestore(app),storage=storageMod.getStorage(app),auth=authMod.getAuth(app);try{if(!auth.currentUser)await authMod.signInAnonymously(auth);}catch(e){console.warn('Anonymous auth not available',e)}firebase={appMod,fsMod,storageMod,authMod,app,db,storage,auth};await ensureProjectRemote();subscribeFirebase();}
