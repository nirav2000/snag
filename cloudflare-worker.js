const WORKER_BUILD='2026.09.24.1005';
...
async function appMonitorCredential(request,env){
  ...
  const hash=await sha256(key);if(hash===APP_MONITOR_ADMIN_SHA256)return {ok:true,hash,master:true};
  const obj=await env.SNAG_MEDIA.get('_app-monitor/v1/_tokens/'+hash+'.json');
  return {ok:!!obj,hash,master:false};
}
async function appMonitorAdmin(request,env){return (await appMonitorCredential(request,env)).ok}
